...
// 等价于 求 最大公共子序列长度L，最终结果是：word1-L + word2-L
int minDistance_dp(string word1, string word2)
{
    ...
    // dp[i][j] 表示 [0,i-1]A 和 [0,j-1]B 的最大公共子序列长度
    dp[0][0] = 0;
...
int minDistance_dpv2(string word1, string word2)
{
    int n = word1.size();
    int m = word2.size();
    // dp[i][j] 表示[0,i-1]的A和[0,j-1]的B 最小删除步数
    vector<vector<int>> dp(n + 1, vector<int>(m + 1, 0));
    // 初始化
    dp[0][0] = 0;
    for(int i =0;i<=n;i++)
